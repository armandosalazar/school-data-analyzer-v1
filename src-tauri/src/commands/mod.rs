...
#[tauri::command]
pub fn upload_file(path: &str) -> Result<Response, Response> {
    ...
    Err(Response {
        success: false,
        message: "Error creating divisions".to_string(),
    })?;


    let mut conn = database::establish_connection();
    match store::crate_teachers(&df, &mut TeacherRepository::new(&mut conn)) {
        Ok(_) => {}
        Err(e) => Err(
            Response {
                success: false,
                message: format!("Error creating teachers: {:?}", e),
            }
        )?,
    }
...
fn create_grades(
    ...
    if df.clone().schema()?.index_of("calificacion1").is_some()
        & df.clone().schema()?.index_of("calificacion2").is_some()
        & df.clone().schema()?.index_of("calificacion3").is_some()
    {
        let result = df
            .clone()
            .lazy()
        ...
        for i in 0..result.height() {
            ...
            let subject_id: Option<i32> = crate::schema::subjects::table
                .filter(
                    crate::schema::subjects::code
                        .eq(result.column("clave")?.str()?.get(i).unwrap())
                        .and(crate::schema::subjects::teacher_id.eq(teacher_id.unwrap())),
                )
